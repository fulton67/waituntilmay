  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 28px", flexShrink: 0 }}>
        <span style={{ fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", color: "#ccc" }}>
          {idx + 1} / {items.length}
        </span>
        <button
          onClick={onClose}
          style={{ background: "none", border: "none", cursor: "pointer", fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", color: "#bbb", fontFamily: "'Courier New', Courier, monospace" }}
        >
          list
        </button>
      </div>
      ...
      {/* Media */}
      <div
        style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 60px", minHeight: 0, transition: "opacity 0.18s ease, transform 0.18s ease", opacity, transform }}
      >
      ...
      {/* Info + nav */}
      <div
        style={{ flexShrink: 0, padding: "20px 28px 32px", display: "flex", justifyContent: "space-between", alignItems: "flex-end", transition: "opacity 0.18s ease", opacity }}
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ fontSize: 12, letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: item.role ? 4 : 0, lineHeight: 1.5 }}>
            {item.title}
          </p>
          {item.role && (
            <p style={{ fontSize: 10, letterSpacing: "0.05em", color: "#aaa" }}>{item.role}</p>
          )}
          <button
            onClick={() => router.push(`/work/${makeSlug(item)}`)}
            style={{ marginTop: 12, background: "none", border: "none", padding: 0, fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", color: "#999", cursor: "pointer", fontFamily: "'Courier New', Courier, monospace", textDecoration: "underline", textUnderlineOffset: 4 }}
          >
            view piece →
          </button>
        </div>
        ...
        <div style={{ display: "flex", gap: 20, paddingLeft: 24, flexShrink: 0 }}>
          <button
            onClick={() => go("left")}
            disabled={idx === 0}
            style={{ background: "none", border: "none", cursor: idx === 0 ? "default" : "pointer", fontSize: 18, color: idx === 0 ? "#e0e0e0" : "#999", fontFamily: "'Courier New', Courier, monospace", padding: 0 }}
          >
            ‹
          </button>
          <button
            ...
            style={{ background: "none", border: "none", cursor: idx === items.length - 1 ? "default" : "pointer", fontSize: 18, color: idx === items.length - 1 ? "#e0e0e0" : "#999", fontFamily: "'Courier New', Courier, monospace", padding: 0 }}
          >
            ›
          </button>
        </div>
...
function GalleryFrame({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ display: "flex", justifyContent: "center", padding: "16px 0 8px" }}>
      <div style={{ width: "min(60vw, 280px)", aspectRatio: "8.5 / 11", background: "#f9f9f9", border: "1px solid #e5e5e5", overflow: "hidden" }}>
        {children}
      </div>
    </div>
  );
...
function MediaExpanded({ item }: { item: WorkItem }) {
    ...
    <div>
      {items.map(item => {
        ...
        return (
          <div key={item.id}>
            <div
              onClick={() => hasMedia && setExpanded(p => p === item.id ? null : item.id)}
              style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, padding: "12px 0", borderBottom: "1px solid #f0f0f0", cursor: hasMedia ? "pointer" : "default" }}
            >
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                {item.role && (
                  <p style={{ fontSize: 10, letterSpacing: "0.05em", color: "#aaa", lineHeight: 1.5, marginTop: 2, marginBottom: 0 }}>
                    {item.role}
                  </p>
                )}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0, paddingTop: 2 }}>
                ...
                {hasMedia && <span style={{ fontSize: 12, color: "#ccc", lineHeight: 1, userSelect: "none" }}>{isOpen ? "−" : "+"}</span>}
              </div>
            </div>
            {isOpen && (
              <div>
                <MediaExpanded item={item} />
                <div style={{ paddingBottom: 8, textAlign: "center" }}>
                  <button
                    onClick={() => router.push(`/work/${makeSlug(item)}`)}
                    style={{ background: "none", border: "none", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", color: "#bbb", cursor: "pointer", fontFamily: "'Courier New', Courier, monospace", textDecoration: "underline", textUnderlineOffset: 3 }}
                  >
                    view piece →
                  </button>
                </div>
...
function PrefaceItem({ item }: { item: WorkItem }) {
  ...
  const router = useRouter();
  return (
    <div style={{ paddingBottom: 20, marginBottom: 8, borderBottom: "1px solid #efefef" }}>
      <p style={{ fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", color: "#c0c0c0", marginBottom: 10 }}>
        preface
      </p>
      <div onClick={() => hasMedia && setOpen(o => !o)} style={{ cursor: hasMedia ? "pointer" : "default", display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 16 }}>
        <p style={{ fontSize: 12, letterSpacing: "0.06em", lineHeight: 1.9, fontStyle: "italic", color: "#444", wordBreak: "break-word" }}>
          {item.title}
        </p>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          {item.year && <span style={{ fontSize: 10, color: "#ccc" }}>{item.year}</span>}
          {hasMedia && <span style={{ fontSize: 11, color: "#bbb" }}>{open ? "−" : "+"}</span>}
        </div>
      </div>
      {open && (
        <div>
      ...
      <div style={{ width: "100%", maxWidth: 560, margin: "0 auto", padding: "0 20px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, marginBottom: 28 }}>
          <button
            ...
            style={{ background: "none", border: "1px solid #e0e0e0", padding: "8px 20px", fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", cursor: "pointer", color: "#555", fontFamily: "'Courier New', Courier, monospace" }}
          >
            view all →
          </button>
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", justifyContent: "center" }}>
            {CATEGORIES.map(cat => (
              <button
                ...
                style={{ background: "none", border: "none", cursor: "pointer", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", fontFamily: "'Courier New', Courier, monospace", color: category === cat.id ? "#000" : "#bbb", textDecoration: category === cat.id ? "underline" : "none", textUnderlineOffset: 4 }}
              >
                {cat.label}
              </button>
            ))}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 36 }}>
          {category === "all" ? (
            SECTION_ORDER.map(id => {
              const catLabel = CATEGORIES.find(c => c.id === id)!.label;
              ...
              return (
                <div key={id}>
                  <p style={{ fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", color: "#ccc", marginBottom: 10 }}>{catLabel}</p>
                  {preface && <PrefaceItem item={preface} />}
                  {rest.length > 0 && <WorkSection items={rest} />}
                </div>
              );
            })
          ) : (
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button
            onClick={copyLink}
            style={{ background: "none", border: "none", cursor: "pointer", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", color: "#bbb", fontFamily: "'Courier New', Courier, monospace" }}
          >

...
export default function CloudView({ items, onSelect }: { items: WorkItem[]; onSelect: (item: WorkItem) => void }) {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(([e]) => {
      ...
      if (w > 0 && h > 0) setSize(s => s && Math.abs(s.w-w)<2 && Math.abs(s.h-h)<2 ? s : { w, h });
    });
    ro.observe(el);
  ...
  useEffect(() => {
    ...
    const { w, h } = size;
    const cx = w/2, cy = h/2;
    const nodes: Node[] = items.map(i => ({ id: i.id, x: cx+(Math.random()-.5)*w*.55, y: cy+(Math.random()-.5)*h*.55 }));
    simRef.current = forceSimulation(nodes)
      .force("c", forceCollide(PAD))
      .force("g", forceCenter(cx, cy))
      .force("m", forceManyBody().strength(-20))
      .alphaDecay(0.035)
      .on("tick", () => {
  ...
  useEffect(() => {
    ...
    const fn = (e: WheelEvent) => {
      ...
      const ns = Math.max(0.15, Math.min(5, scale*(1 + -d*0.0005)));
  ...
  return (
    <div
      ref={wrapRef}
      style={{ position:"absolute", inset:0, overflow:"hidden", cursor:"grab", touchAction:"none", zIndex:1 }}
      onPointerDown={e => {
    ...
    >
      <div style={{ position:"absolute", top:0, left:0, width: size?.w ?? "100%", height: size?.h ?? "100%", transform:`translate(${vp.x}px,${vp.y}px) scale(${vp.scale})`, transformOrigin:"0 0", willChange:"transform" }}>
        {items.map(item => {
          ...
          return (
            <div
              ...
              style={{ position:"absolute", width:ITEM_W, left:p.x-ITEM_W/2, top:p.y-PAD/2, background:color(item.id), cursor:"pointer", overflow:"hidden", transition:"transform 0.15s ease" }}
            >
              {src
